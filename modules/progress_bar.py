...
class ProgressPercentage(object):
    """ 
    Script will track uploading progress of each data file being transferred to cloud storage.
    ...
    def __init__(self, file_path):
        """
        Args: 
            file_path (str): Data file's absolute directory path 
                             (incl. filename & extension).
            
        """
        
        # File's full directory path (incl. filename).
        self.file_path = file_path
        
        # File's size. Note: Calculated via powers of 1000 rather than 1024.
        self.size = float(os.path.getsize(file_path))
        
        # Initialize counter for file size, which details the file size that has
        # already been uploaded at the given time.
        self.seen_so_far = 0
    ...
    def __call__(self, bytes_amount):
        ...
        # Once worker threads are locked, cumulate the counter for file size.
        with self.lock:
            
            # Cumulative file sizes (in bytes). 
            # Note: Calculated via powers of 1000 -- rather than 1024.
            self.seen_so_far += bytes_amount
            
            # Percentage of the file's uploading progress.
